...
public class RecommendServiceImpl implements RecommendService {
    ...
    public List<UserGoodsScore> getUserComment() {
        return userGoodsScoreMapper.selectList(new QueryWrapper<>());
    }
    ...
    public void updateMatrix(Long userId, Long goodsId, double score) {
        ...
        if (!matrix.containsKey(userId)) {
            Long fuserId = matrix.keySet().iterator().next();
        ...
        if (!map.containsKey(goodsId)) {
            matrix.values().forEach(userRatings -> userRatings.put(goodsId, 0.0));
        }
    ...
    private void updateUserProfiles() {
        ...
        // 保存用户画像到Redis
        redisTemplate.opsForValue().set("userProfiles", userProfiles);
    }
    ...
    private long calculateTimeSpan(List<UserGoodsScore> userComments) {
        ...
        return userComments.stream()
            .mapToLong(comment -> comment.getCreateTime().getTime())
            .max()
