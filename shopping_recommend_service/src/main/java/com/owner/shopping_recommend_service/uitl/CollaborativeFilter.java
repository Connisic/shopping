...
public class CollaborativeFilter {
    ...
    private final ThreadPoolExecutor ioThreadPool = new ThreadPoolExecutor(
            Runtime.getRuntime()
                    .availableProcessors() * 2, // 核心线程数：IO密集型任务通常设置为CPU核心数的2倍
            Runtime.getRuntime()
                    .availableProcessors() * 4, // 最大线程数：允许更多线程处理IO等待
            60L, // 空闲线程存活时间
    ...
    private final ThreadPoolExecutor cpuThreadPool = new ThreadPoolExecutor(
            Runtime.getRuntime()
                    .availableProcessors() + 1, // 核心线程数：CPU密集型任务通常设置为CPU核心数+1
            Runtime.getRuntime()
                    .availableProcessors() + 1, // 最大线程数：与核心线程数相同，避免频繁上下文切换
            60L,
            TimeUnit.SECONDS,
            new LinkedBlockingQueue<>(500),
            new ThreadFactoryBuilder().setNameFormat("cpu-pool-%d")
                    .build(),
    ...
    @PostConstruct
    public void init() {
        userItemMap = (Map<Long, Map<Long, Double>>) redisTemplate.opsForValue()
                .get("matrix");
        itemSimilarity = (Map<Long, Map<Long, Double>>) redisTemplate.opsForValue()
                .get("similarity");
        if (userItemMap == null) {
            userItemMap = generateUserItemMap();
        }
        if (itemSimilarity == null) {
            itemSimilarity = generateItemSimilarity();
        }
    }
    ...
    public Map<Long, Map<Long, Double>> calculateItemSimilarity() {
        ...
        Set<Long> goodsSet = userItemMap.values()
                .iterator()
                .next()
                .keySet();
        ...
        CompletableFuture.runAsync(() -> {
                ...
                .join(); // 使用IO密集型线程池，因为Redis操作是IO密集型任务
    ...
    private List<Long> getDefaultRecommendations(int numItems) {
        // 从Redis获取商品热度数据
        Map<Long, Double> itemPopularity = (Map<Long, Double>) redisTemplate.opsForValue()
                .get("item:popularity");
    ...
    private Map<Long, Double> generateItemPopularity() {
        ...
        redisTemplate.opsForHash()
                .putAll("item:popularity", popularity);
    ...
    private double calculateSparsity(Map<Long, Double> userRatings) {
        ...
        long totalItems = userItemMap.values()
                .iterator()
                .next()
                .size();
        long ratedItems = userRatings.values()
                .stream()
    ...
    private List<Long> getPopularItems(int numItems, List<Long> categoryTypes) {
        ...
        List<Long> allResult = itemPopularity.entrySet()
                .stream()
                .sorted(Map.Entry.<Long, Double>comparingByValue()
                        .reversed())
                .map(Map.Entry::getKey)
    ...
    private double getItemAge(Long itemId) {
        ...
        Double cachedAge = (Double) redisTemplate.opsForValue()
                .get(cacheKey);
        ...
        return (double) (new Date().getTime() - goods.getCreateTime()
    ...
    private Map<Long, Double> getItemAgesBatch(Collection<Long> itemIds) {
        ...
        List<Object> cachedAges = redisTemplate.opsForValue()
                .multiGet(cacheKeys);
                    ...
                    try {
                        ...
                        if (!cacheData.isEmpty()) {
                            for (Map.Entry<String, Object> entry : cacheData.entrySet()) {
                                redisTemplate.opsForValue()
                                        .set(entry.getKey(), entry.getValue(), 24, TimeUnit.HOURS);
                            }
                        }
                    } catch (Exception e) {
    ...
    private Map<String, Object> getUserProfile(Long userId) {
        Map<Long, Map<String, Object>> userProfiles = (Map<Long, Map<String, Object>>) redisTemplate.opsForValue()
                .get("userProfiles");
    ...
    private double calculateDiversityScore(Long goodsId, List<Long> selectedItems) {
        ...
        for (Long selectedItem : selectedItems) {
            totalSimilarity += itemSimilarity.get(goodsId)
                    .get(selectedItem);
        }
    ...
    private double calculatePredictedRating(Map<Long, Double> userRatings, Long goodsId) {
        ...
        for (Long i : userRatings.keySet()) {
            ...
            if (userRatings.get(i) > 0) {
                weightedSum += itemSimilarity.get(goodsId)
                        .get(i) * userRatings.get(i);
                similaritySum += Math.abs(itemSimilarity.get(goodsId)
                        .get(i));
            }
        }
        // 避免除零错误
    ...
    private List<Long> getCollaborativeRecommendations(Map<Long, Double> userRatings, int numItems) {
        ...
        // 获取所有商品ID
        Set<Long> allItems = userItemMap.values()
                .iterator()
                .next()
                .keySet();
        ...
        CompletableFuture<List<Long>> diverseRecommendationsFuture =
                CompletableFuture.supplyAsync(() -> getDiverseRecommendations(predictedRatings, numItems),
                        cpuThreadPool);

        return diverseRecommendationsFuture.join();
    }

    private Map<Long, Map<Long, Double>> generateUserItemMap() {
        // 从RecommendServiceImpl中移植generateMatrix的逻辑
        // 获取所有用户评论数据
        List<UserGoodsScore> userComments = getUserComments();
        // 获取所有用户
        List<ShoppingUser> userList = getUserList();
        // 获取所有商品
        List<Goods> allGoods = goodsService.findAll();
        ...
        userList.parallelStream()
                .forEach(user -> {
                    // 为每个用户创建商品评分映射
                    Map<Long, Double> userRatings = new ConcurrentHashMap<>();
                    // 初始化所有商品评分为0
                    allGoods.forEach(goods -> userRatings.put(goods.getId(), 0.0));
                    // 填充用户实际评分数据
                    userComments.stream()
                            .filter(comment -> comment.getUserid().equals(user.getId()))
                            .forEach(comment -> userRatings.put(comment.getGoodsid(), comment.getScore()));
                    // 将用户评分数据添加到矩阵
                    matrix.put(user.getId(), userRatings);
                });
    ...
    private List<UserGoodsScore> getUserComments() {
        // 尝试从缓存中获取用户评分数据
        List<UserGoodsScore> userComments = (List<UserGoodsScore>) redisTemplate.opsForValue()
                .get("user:comments");
                
        // 如果缓存中没有数据，从数据库获取
        if (userComments == null) {
            // 直接从数据库查询所有用户评分数据
            userComments = userGoodsScoreMapper.selectList(null);
            
            // 缓存用户评分数据，设置过期时间为1小时
            if (userComments != null && !userComments.isEmpty()) {
                redisTemplate.opsForValue().set("user:comments", userComments);
                redisTemplate.expire("user:comments", 1, TimeUnit.HOURS);
            }
        }
        
        return userComments != null ? userComments : new ArrayList<>();
    }
    
    /**
     * 获取所有用户列表
     */
    private List<ShoppingUser> getUserList() {
        // 尝试从缓存中获取用户列表
        List<ShoppingUser> userList = (List<ShoppingUser>) redisTemplate.opsForValue()
                .get("user:list");
                
        // 如果缓存中没有数据，从用户服务获取
        if (userList == null) {
            userList = shoppingUserService.getAllUser();
            // 缓存用户列表，设置过期时间为1小时
            if (userList != null && !userList.isEmpty()) {
                redisTemplate.opsForValue().set("user:list", userList);
                redisTemplate.expire("user:list", 1, TimeUnit.HOURS);
            }
        }
    ...
    private Map<Long, Map<Long, Double>> generateItemSimilarity() {
        // 确保用户-物品评分矩阵已经存在
        if (userItemMap == null || userItemMap.isEmpty()) {
            userItemMap = generateUserItemMap();
        }
        
        // 如果用户-物品矩阵为空（可能是因为没有用户评分数据），返回空矩阵
